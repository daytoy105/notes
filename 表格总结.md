...
| ref      | 字符串：this.refs.name <br> 回调函数  | 取消字符串的方式<br> 创建：this.myRef = React.createRef() <br> 获取DM：this.myRef.current <br> `转发ref`：React16以前，ref不是能向下传递的。React 16新增加`forwardRef` |
...
| ----------|------------------|------------------|
| `<Router>`<br>`<Route>`标签| 引入：import { Router, Route, IndexRoute, hashHistory } from 'react-router'<br> `<Router>`可有多个子节点<Route> |React-Rtouter4将react-router-dom拆分 <br> 引入：import { HashRouter as Router, Route } from 'react-router-dom';<br> `<Router>`只能有一个子节点\<Route>，多个需要用<div>包裹 |
...
| 路由跳转 | class property: static contextTypes:{router: React.PropTypes.object}<br>使用：this.context.router.push(path)  | this.props.history.push(path) <br> 这个组件没有由\<Route>直接渲染，可以使用`withRouter()`高阶组件获取history对象 |
...
| 环境定义 | DefinePlugin <br>new webpack.DefinePlugin({'process.env': {'NODE_ENV': JSON.stringify(nodeEnv)}}) | 同 V1 | 移除DefinePlugin，module.exports配置mode |
...
| 表单 | `v-model`: 双向绑定 (value/input结合), 根据控件类型自动选取正确的方法来更新元素<br>忽略所有表单元素的 value、checked、selected特性的初始值而总是将Vue实例的数据作为数据来源 |1. 受控组件, `value` / `checked` , `onChange()` <br> 2. 非受控组件, `ref`、`defaultValue`|
| 组件 |
...
| 组件的使用 | 1. 全局注册，需要在vue实例创建之前发生<br> 2. 局部注册，components属性中指定使用的组件名 | 1. 组件直接嵌套<br>2. 高阶组件 @Hoccomponent <br>3. React.createElement() <br> 4. React.createPortal() |
...
| 混入 | `mixins`属性<br>1. 数据对象：在内部进行浅合并 (一层属性深度)，在和组件的数据发生冲突时以`组件数据优先`<br> 2. 生命周期钩子：同名钩子函数将混合为一个数组，因此都将被调用。另外，混入对象的钩子将在组件自身钩子之前调用。<br>3. 普通函数：两个对象键名`冲突`时，取`组件对象`的键值对<br> 全局混入：` Vue.mixin()`| 高阶组件 |
...
|  方法  | 用法及其优缺点 |
|-----|------|
| cros | CORS(Cross-Origin Resource Sharing, 跨源资源共享),背后的基本思想，就是使用自定义的HTTP头部让浏览器与服务器进行沟通，从而决定请求或响应是应该成功，还是应该失败。<br> 服务器如果认为这个请求可以接受，就在`Access-Control-Allow-Origin`头部中回发相同的源信息(如果是公共资源，可以是`'*'`) |
...
|-----|-------|----------|-----------|
| svn |  | | |
| git | 1. git remote add origin https://github.com/zhanglizhen0/notes.git <br> 2. git push -u origin master <br> 3. git pull https://github.com/zhanglizhen0/notes.git <br> | | |
